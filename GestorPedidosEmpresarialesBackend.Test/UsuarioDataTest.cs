...
namespace GestorPedidosEmpresarialesBackend.Test
{
    ...
    public class OrdenDataTest
    {
        ...
        public async Task Setup()
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var cmd = new SqlCommand(@"
                    ...
                    VALUES (@idProductoBase, 'M', 'Variante test', 1500, 10, 0);
                    SELECT SCOPE_IDENTITY();", connection))
                {
        ...
        public void InsertarOrdenConDetallesYObtenerOrdenes_Success()
        {
            ...
            Assert.IsNotNull(ordenInsertada, "No se recuperó la orden insertada.");
            Assert.AreEqual("Test Direccion", ordenInsertada.DireccionViaje);
            Assert.That(ordenInsertada.DetallesOrden, Is.Not.Empty, "Debe haber detalles en la orden.");
            Assert.AreEqual(2, ordenInsertada.DetallesOrden[0].Cantidad);
        }
    }
}
